	...
	private function giveKit(Session $session, bool $firstPlayer = true) : void{
		...
		if($kit !== null){
			...
			foreach($inventoryContents as $slot => $item){
				if($item->getTypeId() === VanillaBlocks::STAINED_CLAY()->asItem()->getTypeId()){
					$inventoryContents[$slot] = VanillaBlocks::STAINED_CLAY()->setColor($firstPlayer ? DyeColor::BLUE() : DyeColor::RED())->asItem()->setCount($item->getCount());
				}
			}
